...
CREATE INDEX foods_name_trgm_idx ON public.foods USING gin (name_normalized gin_trgm_ops);
CREATE INDEX foods_brand_idx ON public.foods (brand);
...
CREATE INDEX food_synonyms_alias_trgm_idx ON public.food_synonyms USING gin (alias_normalized gin_trgm_ops);
...
CREATE TABLE public.food_search_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX food_search_history_user_idx ON public.food_search_history (user_id, last_used_at DESC);
CREATE UNIQUE INDEX food_search_history_user_food_idx ON public.food_search_history (user_id, food_id) WHERE food_id IS NOT NULL;
...
CREATE INDEX food_favorites_user_idx ON public.food_favorites (user_id);
...
CREATE TABLE public.food_custom_foods (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  food_id UUID REFERENCES public.foods(id) ON DELETE SET NULL,
  name TEXT NOT NULL,
...
CREATE INDEX food_custom_foods_user_idx ON public.food_custom_foods (user_id);
