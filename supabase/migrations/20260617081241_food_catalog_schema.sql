...
CREATE POLICY "barcodes readable by authenticated" ON public.food_barcodes FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "synonyms readable by authenticated" ON public.food_synonyms FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "servings readable by authenticated" ON public.food_servings FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "quality readable by authenticated" ON public.food_quality_scores FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "users manage own search history" ON public.food_search_history FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "users manage own favorites" ON public.food_favorites FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "users manage own custom foods" ON public.food_custom_foods FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP TRIGGER IF EXISTS foods_touch_updated_at ON public.foods;
CREATE TRIGGER foods_touch_updated_at BEFORE UPDATE ON public.foods FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();
DROP TRIGGER IF EXISTS food_custom_foods_touch_updated_at ON public.food_custom_foods;
CREATE TRIGGER food_custom_foods_touch_updated_at BEFORE UPDATE ON public.food_custom_foods FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();
