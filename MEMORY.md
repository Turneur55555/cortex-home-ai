...
- État déploiement : projet `bcwfvpwxzlmkxobvbtzp`, ACTIVE, verify_jwt. ⚠️ Le MCP Supabase a été **instable** pendant la session (déconnexions répétées `deploy_edge_function`). La v1 déployée = code auto-contenu fonctionnellement identique au repo ; un dernier redéploiement pour resynchroniser **les commentaires d'en-tête** à l'identique a échoué sur une déconnexion MCP → à relancer quand le MCP est stable (ou via Publish Lovable / `supabase functions deploy analyze-exercise`). Aucune conséquence fonctionnelle.
