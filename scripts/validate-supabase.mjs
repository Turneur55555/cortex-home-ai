...
function writeSql(name, content) {
  writeFileSync(join(MIG_DIR, name), content, "utf8");
}
...
for (const m of migrations) {
  ...
  if (/\bCREATE(?:\s+UNIQUE)?\s+INDEX\b(?!\s+IF NOT EXISTS)/i.test(sql)) {
    ...
    if (
      ...
      patch(m.name, (s) =>
        s.replace(
          /\bCREATE(\s+UNIQUE)?\s+INDEX\b(?!\s+IF NOT EXISTS)/gi,
...
for (const m of migrations) {
  ...
  if (/\bCREATE FUNCTION\b/i.test(sql)) {
    ...
    if (
      FIX &&
      patch(m.name, (s) => s.replace(/\bCREATE FUNCTION\b/gi, "CREATE OR REPLACE FUNCTION"))
    )
...
if (fixed.length) {
  ...
  for (const f of fixed) console.log(c.dim(`   [FIX] ${f.code.padEnd(26)} ${f.file}`));
  console.log("");
}

if (warnings.length) {
  console.log(c.yellow(`⚠️  Avertissements non corrigés : ${warnings.length}`));
  for (const w of warnings) console.log(c.yellow(`   [${w.code}] ${w.file}: ${w.msg}`));
  console.log("");
}

if (errors.length) {
  console.log(c.red(`❌ Erreurs critiques : ${errors.length}`));
  for (const e of errors) console.log(c.red(`   [${e.code}] ${e.file}: ${e.msg}`));
  console.log("");
  process.exit(1);
}
