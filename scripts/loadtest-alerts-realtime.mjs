...
while (received.INSERT + received.UPDATE < expectedTotal && Date.now() < drainDeadline) {
...
// ── Phase 4 : cleanup ───────────────────────────────────────────────────────
console.log("\n▶ Cleanup");
const { error: delErr } = await supabase.from("items").delete().in("id", createdIds);
...
console.log(
  `  évts reçus         : ${totalReceived}   (INSERT ${received.INSERT} / UPDATE ${received.UPDATE})`,
);
