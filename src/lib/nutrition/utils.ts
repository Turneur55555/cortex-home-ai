import { resolveConsumedGrams } from "./weight";
...
/**
 * Badge de poids affiché sur une ligne du journal — toujours en grammes.
 * `null` si le poids réel est irrécupérable (lignes historiques ambiguës) :
 * on préfère ne rien afficher plutôt qu'une valeur fausse.
 */
export function getWeightBadge(entry: {
  consumed_quantity?: number | null;
  consumed_unit?: string | null;
  consumed_grams_per_unit?: number | null;
}): string | null {
  const grams = resolveConsumedGrams(entry);
  ...
  try {
    const [img, orientation] = await Promise.all([
      new Promise<HTMLImageElement>((res, rej) => {
        ...
        i.onerror = () => {
          clearTimeout(t);
          rej(new Error("Format image non supporté par le navigateur"));
        };
        i.src = dataUrl;
      }),
